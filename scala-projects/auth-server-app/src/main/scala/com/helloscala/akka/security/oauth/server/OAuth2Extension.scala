...
class OAuth2Extension()(implicit val system: ActorSystem[_]) extends Extension {
  ...
  def jwtEncoder: EntityRef[JwtEncoder.Command] = serverConfigure.jwtEncoder

  def keyManager: EntityRef[KeyManager.Command] = serverConfigure.keyManager

  def authorizationService: EntityRef[OAuth2AuthorizationService.Command] = serverConfigure.authorizationService
